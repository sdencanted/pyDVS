...
if len(sys.argv) != 7:
    print("Usage:")
    print("\tpython sequence_to_kronecker.py images_dir output_dir  width height frames_per_second use_adaptive_threshold\n\n")
...
def update_ref(output_type, abs_diff, spikes, ref, thresh, frame_time_ms,
               num_spikes=1, history_weight=1., log2_table=None, use_adaptive_threshold=False):
    if use_adaptive_threshold:
        ...
        if output_type == OUTPUT_RATE:
            return gs.update_reference_rate_adpt(abs_diff, spikes, ref, thresh,
...
use_adaptive_threshold = sys.argv[6] not in ["False", "0"]
print("using adaptive threshold? %s" % (use_adaptive_threshold))
if use_adaptive_threshold:
...
for img_idx in range(num_images):
    ...
    orig_img[:] = cv2.resize(cv2.imread(
        filename, cv2.IMREAD_GRAYSCALE), (cam_w, cam_h))
    padd_img[:] = 0
...
#     curr *= fade_mask
    if use_adaptive_threshold:
        diff[:], abs_diff[:], spikes[:] = gs.thresholded_difference_adpt(
            curr, ref, thresh_matrix)
        # accumulated image
        ref[:],thresh_matrix = update_ref(output_type, abs_diff, spikes, ref, thresh_matrix, frame_time_ms,
                            num_bins, history_weight, log2_table, use_adaptive_threshold)
    else:
    ...
    #     out_dir_name, f'frame_{img_idx:010d}.png'), kronecker_img)
    cv2.imwrite(os.path.join(
        out_dir_name, os.path.basename(filename)), kronecker_img)
